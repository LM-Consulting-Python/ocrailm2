...
def main():
    ...
    with col1:  # Coluna para upload e visualização da imagem
        uploaded_file = st.file_uploader(
            "Carregue o arquivo", type=["pdf", "jpg", "png"], key="file_uploader"
        )

        if uploaded_file is not None:
            file_stream = io.BytesIO(uploaded_file.getvalue())
            with st.spinner("Analisando documento..."):
                result = analyze_document_from_stream(
                    form_recognizer_client, file_stream
                )
    ...
    with col2:  # Coluna para exibição das tabelas e botões de download
        if uploaded_file is not None and hasattr(result, "tables") and result.tables:
            for i, table in enumerate(result.tables):
                ...
                output.seek(0)
                st.download_button(
                    label=f"Baixar Tabela {i+1} como Excel",
                    data=output,
...
def table_to_dataframe(table):
    ...
    df = pd.DataFrame(rows, columns=["Row", "Column", "Content"]).pivot(
        index="Row", columns="Column", values="Content"
    )
    return df
